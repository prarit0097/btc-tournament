...
```
python -m btc_tournament.run_hourly
```

Alias (same behavior, clearer name for 10m setup):

```
python -m btc_tournament.run_10m
```
...
```
python -m btc_tournament.predict
```

5) Run doctor (self-check + smoke test):

```
python -m btc_tournament.doctor
```
...
- Stale data: run `python -m btc_tournament.run_hourly` to refresh cache
